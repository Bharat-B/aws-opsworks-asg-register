...
REGISTER_OPTS="--region $REGION --stack-id $OPSWORKS_STACK_ID --infrastructure-class ec2 --use-instance-profile --local"
...
if [ ! -f "/var/lib/aws/opsworks/setup.done" ]; then
        ...
        #Get Opsworks Compatible Instance Name
        OPSWORKS_HOSTNAME=`aws opsworks get-hostname-suggestion --region $REGION --layer-id=$OPSWORKS_LAYER_ID --output=json | jq -r '.Hostname'`
        ...
        OPSWORKS_INSTANCE_ID=(aws opsworks register $REGISTER_OPTS --override-hostname $OPSWORKS_HOSTNAME | grep -o "Instance ID: .*" | cut -d':' -f2 | tr -d ' ')
